...
# Adjusts replica count and deployment timeout based on K8S_CLUSTER (kind for local clusters, cloud for managed clusters).
# Provides rollout, cleanup, and diagnostic commands while verifying CRDs and operator readiness after installation.
...
declare -A REPLICAS=( ["kind"]="1" ["cloud"]="2" )
declare -A TIMEOUTS=( ["kind"]="120" ["cloud"]="300" )
...
Environment variables:
  K8S_CLUSTER  Cluster type: 'kind' or 'cloud' (default: kind)
...
Examples:
  ...
  K8S_CLUSTER=cloud bash $0 --rollout
EOFHELP
    ;;
